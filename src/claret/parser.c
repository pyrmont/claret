        ...
        if (c != '"') {
            switch (state->argn) {
                case 1:
                    ...
                    state->flags |= PFLAG_STRING;
                    pushstate(p, stringchar, state->flags);
                    push_buf(p, c);
                    return 1;
                case 2:
                    state->flags &= ~PFLAG_LONGSTRING;
...
static int bang(JanetParser *p, JanetParseState *state, uint8_t c) {
    (void) state;
    p->statecount--;
    switch (c) {
        case '{':
            pushstate(p, root, PFLAG_CONTAINER | PFLAG_CURLYBRACKETS | PFLAG_BANG);
            return 1;
        case '"':
            pushstate(p, longstring, PFLAG_BUFFER | PFLAG_LONGSTRING);
            return 1;
        case '[':
            pushstate(p, root, PFLAG_CONTAINER | PFLAG_SQRBRACKETS | PFLAG_BANG);
            return 1;
        case '(':
            pushstate(p, root, PFLAG_CONTAINER | PFLAG_PARENS | PFLAG_BANG);
            return 1;
        default:
            break;
    }
    pushstate(p, tokenchar, PFLAG_TOKEN);
    push_buf(p, '!'); /* Push the leading exclamation mark that was dropped */
    return 0;
}
...
static int directive(JanetParser *p, JanetParseState *state, uint8_t c) {
    (void) state;
    if (p->bufcount == 0 && c != '#') {
        p->statecount--;
        p->bufcount = 0;
...
static void claret_parser_checkdead(JanetParser *parser) {
...
static void claret_parser_consume(JanetParser *parser, uint8_t c) {
    int consumed = 0;
    claret_parser_checkdead(parser);
    if (c == '\r') {
        parser->line++;
        parser->column = 0;
    } else if (c == '\n') {
...
static void claret_parser_eof(JanetParser *parser) {
    claret_parser_checkdead(parser);
    size_t oldcolumn = parser->column;
    size_t oldline = parser->line;
    claret_parser_consume(parser, '\n');
...
static enum JanetParserStatus claret_parser_status(JanetParser *parser) {
...
static void claret_parser_flush(JanetParser *parser) {
    parser->argcount = 0;
    parser->statecount = 1;
    parser->bufcount = 0;
    parser->pending = 0;
...
static const char *claret_parser_error(JanetParser *parser) {
    enum JanetParserStatus status = claret_parser_status(parser);
    if (status == JANET_PARSE_ERROR) {
        const char *e = parser->error;
        parser->error = NULL;
        parser->flag &= ~JANET_PARSER_GENERATED_ERROR;
        claret_parser_flush(parser);
        return e;
    }
    return NULL;
}

static Janet claret_parser_produce(JanetParser *parser) {
    Janet ret;
    size_t i;
...
static void claret_parser_init(JanetParser *parser) {
    parser->args = NULL;
    parser->states = NULL;
    parser->buf = NULL;
    parser->argcount = 0;
    ...
    pushstate(parser, directive, 0);
}

static void claret_parser_deinit(JanetParser *parser) {
    free(parser->args);
    free(parser->buf);
    free(parser->states);
}

static void claret_parser_clone(const JanetParser *src, JanetParser *dest) {
    /* Misc fields */
    dest->flag = src->flag;
...
static int claret_parser_has_more(JanetParser *parser) {
...
static int parsergc(void *p, size_t size) {
    JanetParser *parser = (JanetParser *)p;
    (void) size;
    claret_parser_deinit(parser);
    return 0;
}

static int parserget(void *p, Janet key, Janet *out);

const JanetAbstractType claret_parser_type = {
    "claret/parser",
    parsergc,
    parsermark,
    parserget,
...
static Janet cfun_parse_parser(int32_t argc, Janet *argv) {
    ...
    JanetParser *p = janet_abstract(&claret_parser_type, sizeof(JanetParser));
    claret_parser_init(p);
    return janet_wrap_abstract(p);
}

static Janet cfun_parse_consume(int32_t argc, Janet *argv) {
    janet_arity(argc, 2, 3);
    JanetParser *p = janet_getabstract(argv, 0, &claret_parser_type);
    ...
    for (i = 0; i < view.len; i++) {
        claret_parser_consume(p, view.bytes[i]);
        switch (claret_parser_status(p)) {
...
static Janet cfun_parse_has_more(int32_t argc, Janet *argv) {
    janet_fixarity(argc, 1);
    JanetParser *p = janet_getabstract(argv, 0, &claret_parser_type);
    return janet_wrap_boolean(claret_parser_has_more(p));
}
...
static Janet cfun_parse_byte(int32_t argc, Janet *argv) {
    ...
    claret_parser_consume(p, 0xFF & i);
    return argv[0];
}
...
static Janet cfun_parse_error(int32_t argc, Janet *argv) {
    janet_fixarity(argc, 1);
    JanetParser *p = janet_getabstract(argv, 0, &claret_parser_type);
    const char *err = claret_parser_error(p);
    if (err) {
...
static Janet cfun_parse_clone(int32_t argc, Janet *argv) {
    janet_fixarity(argc, 1);
    JanetParser *src = janet_getabstract(argv, 0, &claret_parser_type);
    JanetParser *dest = janet_abstract(&claret_parser_type, sizeof(JanetParser));
    claret_parser_clone(src, dest);
...
static const JanetReg cfuns[] = {
    {
        "parser", cfun_parse_parser,
        JDOC("(claret/parser)\n\n"
             "Creates and returns a new Claret parser object. Parsers are state "
...
void claret_parser_register_type(JanetTable *env) {
    (void) env;
    janet_register_abstract_type(&claret_parser_type);
}

void claret_parser_register_functions(JanetTable *env) {
    janet_cfuns(env, "claret", cfuns);
}

JANET_MODULE_ENTRY(JanetTable *env) {
    claret_parser_register_type(env);
    claret_parser_register_functions(env);
}
